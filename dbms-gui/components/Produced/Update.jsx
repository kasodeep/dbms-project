...
export default function Update({ produced }) {
  const { toast } = useToast()

  const form = useForm({
    resolver: zodResolver(FormSchema),
    defaultValues: {
      quantity: String(produced[0].quantity),
      identifier: produced[0].identifier,
      rate: String(produced[0].rate),
      worker_id: String(produced[0].worker_id),
    },
  })
  ...
  async function onSubmit(data) {
    ...
    const postData = {
      ...
      body: JSON.stringify({
        produced_id: produced[0].produced_id,
